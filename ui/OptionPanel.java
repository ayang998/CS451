...
public class OptionPanel extends JPanel {
	...
	private void handleNetworkUpdate(NetworkWindow networkWindow, ActionEvent e) {
		...
		if (e.getID() == NetworkWindow.LISTEN_BUTTON) {
			...
			Command connect = new Command(Command.COMMAND_CONNECT, networkWindow.getSourcePort() + "", isPlayer1? "1" : "0");
			String response = connect.send(destinationHost, destinationPort);

			if (response.isEmpty()) {
				networkWindow.setMessage("  Error: could not connect to " + destinationHost + ":" + destinationPort + ".");
				...
				networkWindow.setMessage("  Successfully started a session with " + destinationHost + ":" + destinationPort + ".");
				...
				networkWindow.setMessage("  Error: you tried to connect to a host and port that isn't running a checkers client.");
			}
		}
	}

	private static Player getPlayer(JComboBox<String> playerOptions) {
	...
	private class OptionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source == restartButton) {
				...
				Player player = getPlayer(player1Options);
				...
				Player player = getPlayer(player2Options);
